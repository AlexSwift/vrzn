...
if not NADMOD.Props then
	...
	timer.Create("WarnOtherPPs", 0, 1, function()
		//Print a harmless error if we detect other PPs active, since this often leads to confusion. NPPll still load fine
		//Known CPPI plugins: FPP, SPP, NPP, ULXs UPS
...
function NADMOD.PlayerMakePropOwner(ply,ent)
	print( NADMOD.Users[ply:SteamID()] )
	if !IsValid(ent) or ent:IsPlayer() then return end
...
function NADMOD.CleanupProps(ply, cmd, args)
	...
	if not EntIndex or EntIndex == "" then
		...
		NADMOD.Notify(ply,"Seus props foram limpos. ("..count..")")
...
function NADMOD.CleanName(ply, cmd, args, fullstr)
	...
	NADMOD.Notify(fullstr..": ("..count..") Props limpos")
end
...
function NADMOD.CDP(ply, cmd, args)
	...
	NADMOD.Notify("("..count..") Props de jogadores desconectados foram limpos")
end
concommand.Add("nadmod_cdp",NADMOD.CDP)

function NADMOD.CleanClass(ply,cmd,args)
	if ply:IsValid() and not NADMOD.IsPPAdmin(ply) then return end
	if args[1] == "npc_*" then NADMOD.Notify("NPCS Removidos")
	elseif args[1] == "prop_ragdol*" then NADMOD.Notify("Ragdolls removidos")
	else NADMOD.Notify(args[1].." Removidos")
	end
...
print("[VRZN GAMEMODE] Sistema de proteção [v"..NADMOD.PPVersion.."]")

hook.Add("PhysgunPickup", "Pickup", function(ply, ent)
	if ent:GetClass() == "prop_physics" then
		ent:SetCollisionGroup( COLLISION_GROUP_WORLD )
		-- local entcol = {ent:GetColor()}
		-- ent:SetColor(255,255,255,10)
		ent:SetRenderMode( RENDERMODE_TRANSALPHA )
		ent:SetColor( Color( 255, 255, 255, 100 ) )
	end
end)

hook.Add("PhysgunDrop", "Drop", function(ply, ent)
	if ent:GetClass() == "prop_physics" then
