...
function Panel:Think()
	local curCar = LocalPlayer():GetNWEntity( "CurrentCar" )
	if IsValid( curCar ) and curCar:GetModel() == self.m_tblCar.Model:lower() then
		self.m_pnlPreviewBtn:SetText( "Retrieve" )
	else
		self.m_pnlPreviewBtn:SetText( "View Info" )
	end
end
...
vgui.Register( "SRPSSCarCard", Panel, "EditablePanel" )
...
function Panel:Init()
	...
	self.m_pnlBuyBtn.DoClick = function()
		GAMEMODE.Net:RequestSpawnSSCar( self.m_tblCar.UID, self.m_colCurrentColor, self.m_intCurrentSkin, self.m_tblCurrentGroups )
	end
...
vgui.Register( "SRPSSCarPreview", Panel, "EditablePanel" )
...
function Panel:Init()
	...
	self.m_pnlCanvas = vgui.Create( "SRP_ScrollPanel", self )
	self.m_pnlCarPreview = vgui.Create( "SRPSSCarPreview", self )
	self.m_pnlCarPreview.m_tblParentMenu = self
	self.m_tblCards = {}

	self:AddSearchBar()

	self:SetTitle( "Government Car Spawn" )
end
...
function Panel:Populate()
	...
	for k, data in pairs( cars ) do
		if data.Job ~= "JOB_SSERVICE" then continue end
...
function Panel:CreateCarCard( k, tblData )
	local pnl = vgui.Create( "SRPSSCarCard" )
	pnl:SetCar( tblData )
...
function Panel:OnClose()
	GAMEMODE.Net:SendNPCDialogEvent( "ss_spawn_car_end_dialog" )
end
vgui.Register( "SRPSSCarSpawn", Panel, "SRP_Frame" )