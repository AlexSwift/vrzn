...
Rain.m_tblSounds = {
	...
	RainLoop = "taloslife/weather/crucial_surfacerain_med_loop.wav",
}
	...
	function Rain:PlayThunderFX()
		...
		if CurTime() >= self.m_intLastThunderTime then
			...
			elseif self.m_bFadingOut then--fadeout
				...
				sound.Play( table.Random(self.m_tblSounds.Thunder[math.random(1, 2)]), pos, 150, math.random(80, 130), volume )
			end
...
elseif SERVER then
	function Rain:GamemodeUpdateMapLighting( strPattern, intLightIndex )
		...
		local targetLightIndex = bTime and math.min(intLightIndex, 340) or math.max(1250, intLightIndex)
		local time = CurTime()
...
if CLIENT then
	...
	local rainColor = Color( 220, 220, 255 )
	...
	function Rain:RenderScreenspaceEffects()
		if self.m_intLastLightFX and not self:Indoors() and GAMEMODE.Config.LightSensitiveMode:GetInt() == 0 then
			...
			--[[DrawColorModify{
				...
				["$pp_colour_contrast"] = Lerp( self.m_intRainScaler, 1, 0.9 ),
	...
	function Rain:GamemodeSetupWorldFog( tblFogData )
		tblFogData.Density = math.min( 0.9, Lerp(self.m_intRainScaler, 0, 0.095) +tblFogData.Density )
	end
end
