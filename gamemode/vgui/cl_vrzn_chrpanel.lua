...
vgui.Register( "SRPCharacterPreview2", Panel, "DButton" )
...
function Panel:Init()
	...
	self.m_pnlCharDisplay = vgui.Create( "SRPCharacterPreview2", self )
...
vgui.Register( "SRPNewCharacterPanel2", Panel, "SRP_FramePanel" )
...
vgui.Register( "SRPCharacterCard2", Panel, "DButton" )
...
function Panel:Populate( tblChars )
	...
	for id, data in pairs( tblChars ) do
		local card = vgui.Create( "SRPCharacterCard2", self.m_pnlCharContainer )
...
vgui.Register( "SRPCharacterSelection2", Panel, "SRP_FramePanel" )
...
--- Cards ----------------------------------
--- --------------------------------------------------------------
surface.CreateFont("CS::PlayButton", { font = "Montserrat Regular", extended = false, size = 20, weight = 500, antialias = true } )
surface.CreateFont("CS::CharacterName", { font = "Montserrat Bold", extended = false, size = 32, weight = 500, antialias = true } )
surface.CreateFont("CS::Title", { font = "Montserrat Bold", extended = false, size = 48, weight = 500, antialias = true } )
...
function PANEL:BuildClientModel( id, tbl )
    ...
    CharacterCreationPreviewModel:ResetSequence( CharacterCreationPreviewModel:LookupSequence("pose_standing_02") )
...
function PANEL:EnablePlayButton()
    ...
    CSPlayButton:DockMargin(2 * self:GetWide()/3, 5, 0, 0)
    ...
    CSPlayButton.Paint = function( pnl, w, h )
        ...
        draw.SimpleText("Entrar como " .. self.CharacterTable.Name.First .. " " .. self.CharacterTable.Name.Last, "CS::PlayButton", w/2, h/2, Color(26,26,26,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
end
...
function SetCalcView( pPlayer, vPos, aAngles, iFov )
	...
	view.angles = Angle(12, -90, 0.000000)
	view.fov = 75
...
function PANEL:Init()
    ...
    self.LeftNavButtonsContainer:SetTall( (64 * ( CharacterCount+1 ) ) + ( 30 * ( CharacterCount+1 ) ) )
...
function PANEL:Populate( tblChars )
    ...
    if table.Count( self.CharacterCards ) < GAMEMODE.Config.MaxCharacters then
		...
		local pnlIconBtn = vgui.Create( "SRP_Button",  self.LeftNavButtonsContainer )
        pnlIconBtn:SetText( "New Character" )
        pnlIconBtn:Dock(TOP)
		pnlIconBtn.DoClick = function()
			-- GAMEMODE.Gui:ShowNewCharacterMenu()
		...
		table.insert( self.CharacterCards, pnlIconBtn )
	end
end
