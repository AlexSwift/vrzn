...
function Panel:InitializeButtons()
	...
	for k, v in pairs( GAMEMODE.CarRadio.m_tblStations ) do
		self.m_tblButtons[#self.m_tblButtons +1] = {
			Name = v.Name,
			Cmd = function()
				GAMEMODE.Net:RequestPlayCarRadio( k )
			end,
		}
	end
		...
		self.m_tblButtons[#self.m_tblButtons +1] = {
			...
			Cmd = function()
				GAMEMODE.Gui:StringRequest(
					"Play custom radio station",
					"Enter a shoutcast station ID",
					"114517",
					function( text )
						GAMEMODE.Net:RequestPlayCustomCarRadio( tonumber(text) )
					end,
					function() end,
					"Play"
				)
	...
	self.m_tblButtons[#self.m_tblButtons +1] = {
		Name = "Stop Radio",
		Cmd = function()
			GAMEMODE.Net:RequestStopCarRadio()
		end,
	}
...
function Panel:Paint( intW, intH )
	...
	for k, v in pairs( self.m_tblButtons ) do
		...
		--surface.DrawLine( sx +lx *96, sy +ly *96, sx +lx *128, sy +ly *128 )
...
vgui.Register( "SRPRadioRadialMenu", Panel, "EditablePanel" )